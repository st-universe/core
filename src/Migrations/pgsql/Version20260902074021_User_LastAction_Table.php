...
final class Version20260902074021 extends AbstractMigration
{
    ...
    public function up(Schema $schema): void
    {
        ...
        $this->addSql(<<<'SQL'
            ...
            (user_id, timestamp)
            SELECT u.id, u.lastaction_timestamp
            FROM stu_user u
            ORDER BY u.id ASC
        SQL);
        $this->addSql('ALTER TABLE stu_user DROP lastaction_timestamp');
    }

    public function down(Schema $schema): void
    {
        $this->addSql('ALTER TABLE stu_user ADD lastaction_timestamp INT DEFAULT NULL');
        $this->addSql(<<<'SQL'
            UPDATE stu_user u
            SET lastaction_timestamp = (
                SELECT timestamp
                FROM stu_user_last_action
                WHERE user_id = u.id
            )
        SQL);
        $this->addSql('ALTER TABLE stu_user ALTER lastaction_timestamp SET NOT NULL');
