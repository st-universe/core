...
final class colonyapp extends gameapp {
	...
	protected function showOrbitalManagement() {
		$this->addNavigationPart(new Tuple("?id=".$this->getColony()->getId(),$this->getColony()->getNameWithoutMarkup()));
		$this->addNavigationPart(new Tuple("?id=".$this->getColony()->getId().'&SHOW_ORBITAL_SHIPS=1',_('Orbitalmanagement')));
	...
	protected function showShipRepair() {
		...
		if ($this->getColony()->hasShipyard()) {
			$this->addNavigationPart(new Tuple("?id=".$this->getColony()->getId(),$this->getColony()->getNameWithoutMarkup()));
			$this->addNavigationPart(new Tuple("?id=".$this->getColony()->getId().'&SHOW_SHIP_REPAIR=1&fid='.$this->getField()->getFieldId(),_('Schiffreparatur')));
	...
	protected function upgradeBuilding() {
		...
		foreach($upgrade->getCost() as $key => $obj) {
			if (!array_key_exists($obj->getGoodId(),$storage)) {
				$this->addInformation(sprintf(_("Es werden %d %s benötigt - Es ist jedoch keines vorhanden"),$obj->getAmount(),getGoodName($obj->getGoodId())));
				return;
			}
			if ($obj->getAmount() > $storage[$obj->getGoodId()]->getCount()) {
				$this->addInformation(sprintf(_("Es werden %d %s benötigt - Vorhanden sind nur %d"),$obj->getAmount(),getGoodName($obj->getGoodId()),$storage[$obj->getGoodId()]->getCount()));
				return;
			}
		}

		if ($this->getColony()->getEps() < $upgrade->getEnergyCost()) {
			$this->addInformation(sprintf(_("Zum Bau wird %d Energie benötigt - Vorhanden ist nur %d"),$upgrade->getEnergyCost(),$this->getColony()->getEps()));
			return;
		}
		...
		$alt_building = BuildingFieldAlternative::getByBuildingField($upgrade->getBuilding()->getId(),$this->getField()->getFieldType());
		if ($alt_building) {
		...
		$this->addInformation($upgrade->getDescription()." wird durchgeführt - Fertigstellung: ".$this->getField()->getBuildtimeDisplay(),TRUE);
	}
	...
	protected function buildOnField() {
		...
		foreach($building->getCosts() as $key => $obj) {
			if ($this->getField()->hasBuilding()) {
				if (!array_key_exists($key,$storage) && !array_key_exists($key,$this->getField()->getBuilding()->getCosts())) {
		...
		// Check for alternative building
		$alt_building = BuildingFieldAlternative::getByBuildingField($building->getId(),$this->getField()->getFieldType());
		if ($alt_building) {
			$building = $alt_building->getAlternateBuilding();
		}
	...
	function getEpsBar() {
		if ($this->epsbars === NULL) {
			...
			foreach ($bars as $color => $value) {
				...
				$this->epsbars[] = new Tuple($color,round($width/100*(100/$this->getColony()->getMaxEps()*$value)));
			}
		}
		return $this->epsbars;
	}
	...
	function preChecks(&$target) {
		if (!checkPosition($this->getColony(),$target) || ($target->cloakIsActive() && !$target->ownedByCurrentUser())) {
	...
	protected function beamTo() {
		...
		$this->addInformation(sprintf(_('Die Kolonie %s hat folgende Waren zur %s transferiert'),$this->getColony()->getName(),$target->getName()));
		foreach ($goods as $key => $value) {
			...
			$this->addInformation(sprintf(_("%d %s (Energieverbrauch: %d)"),$count,$good->getGood()->getName(),$eps_usage));
	...
	protected function beamFrom() {
		...
		$this->addInformation(sprintf(_('Die Kolonie %s hat folgende Waren von der %s transferiert'),$this->getColony()->getName(),$target->getName()));
		foreach ($goods as $key => $value) {
			if ($this->getColony()->getEps() < 1) {
				break;
			}
	...
	function performTerraforming() {
		...
		FieldTerraforming::addTerraforming($this->getColony()->getId(),$this->getField()->getId(),$terraf->getId(),$time);
	...
	protected function manageOrbitalShips() {
		...
		foreach ($ships as $key => $ship) {
			...
			if ($this->getColony()->getEps() > 0 && $shipobj->getEBatt() < $shipobj->getMaxEbatt() && array_key_exists($ship,$batt)) {
				if ($batt[$ship] == 'm') {
				...
				if ($load > 0) {
					...
					if (!$shipobj->ownedByCurrentUser()) {
						PM::sendPM(currentUser()->getId(),
							   $shipobj->getUserId(),
							   "Die Kolonie ".$this->getColony()->getName()." lädt in Sektor ".$this->getColony()->getSectorString()." die Batterie der ".$shipobj->getName()." um ".$load." Einheiten",PM_SPECIAL_TRADE);
					}
				}
			}
			if (isset($man[$shipobj->getId()]) && $shipobj->currentUserCanMan()) {
				if ($shipobj->getBuildplan()->getCrew() > $shipobj->getUser()->getFreeCrewCount()) {
					$msg[] = sprintf(_('%s: Nicht genügend Crew vorhanden (%d benötigt)'),$shipobj->getName(),$shipobj->getBuildplan()->getCrew());
				} else {
				...
				if ($storage->offsetExists(GOOD_DEUTERIUM) && $storage->offsetExists(GOOD_ANTIMATTER)) {
					if ($shipobj->getWarpcoreLoad() < $shipobj->getWarpcoreCapacity()) {
						...
						if ($load >= 1) {
							...
							$msg[] = sprintf(_('Der Warpkern der %s wurde um %d Einheiten aufgeladen'),$shipobj->getName(),$load);
							if (!$shipobj->ownedByCurrentUser()) {
								PM::sendPM(currentUser()->getId(),
									   $shipobj->getUserId(),
					...
					$msg[] = sprintf(_('%s: Es wird Deuterium und Antimaterie zum Aufladen des Warpkerns benötigt'),$shipobj->getName());
				}
			}
			if (isset($torp[$shipobj->getId()]) && $shipobj->canLoadTorpedos()) {
				...
				try {
					...
					if ($shipobj->getTorpedoCount() == 0 && (!isset($torp_type[$shipobj->getId()]) || !array_key_exists($torp_type[$shipobj->getId()],$shipobj->getPossibleTorpedoTypes()))) {
						throw new Exception;
					}
					...
					if ($shipobj->getTorpedoCount() > 0) {
						...
						if ($load > 0) {
							if (!$storage->offsetExists($torp_obj->getGoodId())) {
								$msg[] = sprintf(_('%s: Es sind keine Torpedos des Typs %s auf der Kolonie vorhanden'),$shipobj->getName(),$torp_obj->getName());
								throw new Exception;
							}
						...
						if ($load < 0) {
							...
							$msg[] = sprintf(_('%s: Es wurden %d Torpedos des Typs %s vom Schiff transferiert'),$shipobj->getName(),abs($load),$torp_obj->getName());
						} elseif ($load > 0) {
							$this->getColony()->lowerStorage($torp_obj->getGoodId(),$load);
							$msg[] = sprintf(_('%s: Es wurden %d Torpedos des Typs %s zum Schiff transferiert'),$shipobj->getName(),$load,$torp_obj->getName());
						}
					} else {
						...
						$msg[] = sprintf(_('%s: Es wurden %d Torpedos des Typs %s zum Schiff transferiert'),$shipobj->getName(),$count,$torp_obj->getName());
					}
				} catch (Exception $e) {
					// nothing
				}
	...
	protected function showModuleScreen() {
		$this->addNavigationPart(new Tuple("?id=".$this->getColony()->getId(),$this->getColony()->getNameWithoutMarkup()));
		$this->addNavigationPart(new Tuple("?id=".$this->getColony()->getId().'&SHOW_MODULE_SCREEN=1&rump='.$this->getSelectedRump()->getId(),_('Schiffbau')));
	...
	public function getModuleSelectors() {
		...
		for ($i=1;$i<=MODULE_TYPE_COUNT;$i++) {
			if ($i == MODULE_TYPE_SPECIAL) {
				$retv[] = new ModuleSelectorSpecial($i,$this->getColony(),$this->getSelectedRump(),currentUser()->getId(),$this->getSelectedBuildplan());
			} else {
				$retv[] = new ModuleSelector($i,$this->getColony(),$this->getSelectedRump(),currentUser()->getId(),$this->getSelectedBuildplan());
			}
		}
		return $retv;
	}
	...
	protected function buildShip() {
		...
		for ($i=1;$i<=MODULE_TYPE_COUNT;$i++) {
			...
			if ($i != MODULE_TYPE_SPECIAL && $this->getSelectedRump()->getModuleLevels()->{'getModuleMandatory'.$i}() > 0 && count($module) == 0) {
				$this->addInformation(sprintf(_('Es wurde kein Modul des Typs %s ausgewählt'),ModuleType::getDescription($i)));
				return;
			}
			if ($i === MODULE_TYPE_SPECIAL) {
		...
		foreach ($modules as $module) {
			...
			$selector = new ModuleSelector($module->getType(),$this->getColony(),$this->getSelectedRump(),currentUser()->getId());
	...
	private function buildShipRump($rump_id) {
		...
		if ($rump->getEpsCost() > $this->getColony()->getEps()) {
			$this->addInformation(sprintf(_('Es wird %d Energie benötigt - Vorhanden ist nur %d'),$rump->getEpsCost(),$this->getColony()->getEps()));
			return;
		}
		$storage = &$this->getColony()->getStorage();
		foreach ($rump->getBuildingCosts() as $key => $cost) {
			if (!array_key_exists($cost->getGoodId(),$storage)) {
				$this->addInformation(sprintf(_('Es wird %d %s benötigt'),$cost->getCount(),$cost->getGood()->getName()));
				$this->rollbackTransaction();
				return;
			}
			if ($storage[$cost->getGoodId()]->getCount() < $cost->getCount()) {
				$this->addInformation(sprintf(_('Es wird %d %s benötigt - Vorhanden ist nur %d'),$cost->getCount(),$cost->getGood()->getName(),$storage[$cost->getGoodId()]->getCount()));
				$this->rollbackTransaction();
				return;
			}
	...
	protected function startAirfieldShip() {
		...
		if ($this->getColony()->getEps() < 10) {
			$this->addInformation(sprintf(_('Es wird %d Energie benötigt - Vorhanden ist nur %d'),10,$this->getColony()->getEps()));
			return;
		}
		$storage = &$this->getColony()->getStorage();
		...
		foreach ($modules as $module_id => $count) {
			...
			try {
				foreach ($module->getCost() as $cid => $cost) {
					if (!array_key_exists($cost->getGoodId(),$storage)) {
						$prod[] = sprintf(_("Zur Herstellung von %s wird %s benötigt"),$module->getName(),$cost->getGood()->getName());
						throw new Exception;
					}
					if ($storage[$cost->getGoodId()]->getCount() < $cost->getCount()) {
						$prod[] = sprintf(_("Zur Herstellung von %s wird %d %s benötigt"),$module->getName(),$cost->getCount(),$cost->getGood()->getName());
						throw new Exception;
					}
	...
	protected function buildTorpedos() {
		...
		foreach ($torps as $torp_id => $count) {
			...
			$msg[] = sprintf(_('Es wurden %d Torpedos des Typs %s hergestellt'),$count*$torp->getAmount(),$torp->getName());
