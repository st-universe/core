...
class commapp extends gameapp {
	...
	function __construct() {
		parent::__construct($this->default_tpl,"/ Kommunikationsnetzwerk");
	...
	function showPlotKN() {
		...
		$this->addNavigationPart(new Tuple("comm.php?SHOW_PLOT=1&plotid=".$plot->getId(),"Plot: ".$plot->getTitleDecoded()));
		$this->addNavigationPart(new Tuple("comm.php?SHOW_PLOTKN=1&plotid=".$plot->getId(),"Plot: ".$plot->getTitleDecoded()));
	...
	function showPlotDetails() {
		...
		$this->addNavigationPart(new Tuple("comm.php?SHOW_PLOTLIST=1","Plotliste"));
		$this->addNavigationPart(new Tuple("comm.php?SHOW_PLOT=1&plotid=".$this->getRPGPlot()->getId(),"Plot: ".$this->getRPGPlot()->getTitleDecoded()));
	...
	function showOutbox() {
		request::setVar('pmcat',PMCategory::getOrGenSpecialCategory(PM_SPECIAL_PMOUT,currentUser()->getId())->getId());
		$this->showPMCat();
	}

	function showPMCat() {
		$this->setTemplateFile('html/pmcategory.xhtml');
		$this->addNavigationPart(new Tuple("comm.php?SHOW_PM_CAT=1&pmcat=".$this->getPMCategory()->getId(),"Private Nachrichten: ".$this->getPMCategory()->getDescriptionDecoded()));
	...
	function getKNNavigation() {
		if ($this->knnav === NULL) {
			...
			for ($i=$curpage-1;$i<=$curpage+3;$i++) {
				...
				$ret[] = array("page" => $i,"mark" => ($i*KNLIMITER-KNLIMITER),"cssclass" => ($curpage+1 == $i ? "pages selected" : "pages"));
			}
	...
	function getPMNavigation() {
		if ($this->pmnav === NULL) {
			...
			for ($i=$curpage-1;$i<=$curpage+3;$i++) {
				...
				$ret[] = array("page" => $i,"mark" => ($i*PMLIMITER-PMLIMITER),"cssclass" => ($curpage+1 == $i ? "pages selected" : "pages"));
			}
			if ($curpage+1 != $maxpage) {
				$ret[] = array("page" => ">","mark" => ($mark+PMLIMITER),"cssclass" => "pages");
	...
	function addPlotMember() {
		...
		PM::sendPM(currentUser()->getId(),$userId,"Du wurdest dem RPG-Plot '".$plot->getTitleDecoded()."' als Schreiber hinzugefügt");
		$this->addInformation("Der Siedler wurde hinzugefügt");
	}
