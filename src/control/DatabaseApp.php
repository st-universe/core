...
final class DatabaseApp extends gameapp {
	...
	function showCategory() {
		$this->addNavigationPart(new Tuple("database.php?SHOW_CATEGORY=1&cat=".$this->getCategory()->getId(),"Datenbank: ".$this->getCategory()->getDescription()));
	...
	function showEntry() {
		...
		$this->addNavigationPart(new Tuple("database.php?SHOW_CATEGORY=1&cat=".$this->getCategory()->getId(),"Datenbank: ".$this->getCategory()->getDescription()));
		$this->addNavigationPart(new Tuple("database.php?SHOW_ENTRY=1&cat=".$this->getCategory()->getId()."&ent=".$this->getDatabaseEntry()->getId(),"Eintrag: ".$this->getDatabaseEntry()->getDescription()));
		$this->addSpecialVars();
        ...
        function getUserListNavigation() {
                if ($this->ulnav === NULL) {
                        ...
                        for ($i=$curpage-1;$i<=$curpage+3;$i++) {
                                ...
                                $ret[] = array("page" => $i,"mark" => ($i*USERLISTLIMITER-USERLISTLIMITER),"cssclass" => ($curpage+1 == $i ? "pages selected" : "pages"));
                        }
                        if ($curpage+1 != $maxpage) {
                                $ret[] = array("page" => ">","mark" => ($mark+USERLISTLIMITER),"cssclass" => "pages");
                                $ret[] = array("page" => ">>","mark" => $maxpage*USERLISTLIMITER-USERLISTLIMITER,"cssclass" => "pages");
                        }
                        $this->ulnav = $ret;
                }
                return $this->ulnav;
