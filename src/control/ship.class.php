...
class shipapp extends gameapp {
	...
	protected function showShip() {
		$this->addNavigationPart(new Tuple("?id=".$this->getShip()->getId(),$this->getShip()->getNameWithoutMarkup()));
	...
	protected function preChecks(&$target,$colony=FALSE) {
		if (!checkPosition($this->getShip(),$target) || $this->getShip()->getCloakState() || ($colony && $target->getId() == $this->getShip()->getId())) {
	...
	function joinFleet() {
		...
		$this->getShip()->save();
		$this->addInformation(sprintf(_("Die %s ist der Flotte %s beigetreten"),$this->getShip()->getName(),$fleet->getName()));
	}
	...
	protected function activateTraktorBeam() {
		...
		if (($ship->getAlertState() == ALERT_YELLOW || $ship->getAlertState() == ALERT_RED) && !$ship->getUser()->isFriend(currentUser()->getId())) {
			...
			$obj = new ShipSingleAttackCycle($attacker,$this->getShip(),$ship->getFleetId(),$this->getShip()->getFleetId());
			$this->addInformationMergeDown($obj->getMessages());
			PM::sendPM(currentUser()->getId(),$ship->getUserId(),"Die ".$this->getShip()->getName()." versucht die ".$ship->getName()." in Sektor ".$this->getShip()->getSectorString()." mit dem Traktorstrahl zu erfassen. Folgende Aktionen wurden ausgeführt:\n".infoToString($obj->getMessages()),PM_SPECIAL_SHIP);
		}
		if ($ship->shieldIsActive()) {
		...
		if (currentUser()->getId() != $ship->getUserId()) {
			PM::sendPM(currentUser()->getId(),$ship->getUserId(),"Die ".$ship->getName()." wurde in SeKtor ".$this->getShip()->getSectorString()." vom Traktorstrahl der ".$this->getShip()->getName()." erfasst",PM_SPECIAL_SHIP);
		}
		...
		if ($this->getShip()->getTraktorMode() == 2) {
			return;
                ...
                if (currentUser()->getId() != $this->getShip()->getTraktorShip()->getUserId()) {
                        PM::sendPM(currentUser()->getId(),$this->getShip()->getTraktorShip()->getUserId(),"Der auf die ".$this->getShip()->getTraktorShip()->getName()." gerichtete Traktorstrahl wurde in SeKtor ".$this->getShip()->getSectorString()." deaktiviert",PM_SPECIAL_SHIP);
                }
	...
	protected function epsTransfer($load=0) {
		...
		$target->save();
		$this->getShip()->save();
		PM::sendPM(currentUser()->getId(),$target->getUserId(),"Die ".$this->getShip()->getName()." transferiert in SeKtor ".$this->getShip()->getSectorString()." ".$load." Energie in die Batterie der ".$target->getName(),PM_SPECIAL_TRADE);
	...
	protected function beamTo() {
		...
		$this->addInformation(sprintf(_('Die %s hat folgende Waren zur %s transferiert'),$this->getShip()->getName(),$target->getName()));
		foreach ($goods as $key => $value) {
			...
			$this->addInformation(sprintf(_('%d %s (Energieverbrauch: %d)'),$count,$good->getGood()->getName(),ceil($count/$good->getGood()->getTransferCount())));
	...
	protected function beamFrom() {
		...
		$this->addInformation(sprintf(_('Die %s hat folgende Waren von der %s transferiert'),$this->getShip()->getName(),$target->getName()));
		foreach ($goods as $key => $value) {
			if ($this->getShip()->getEps() < 1) {
				break;
			}
	...
	function beamToColony() {
		...
		$this->addInformation(sprintf(_('Die %s hat folgende Waren zur Kolonie %s transferiert'),$this->getShip()->getName(),$target->getName()));
		foreach ($goods as $key => $value) {
			if ($this->getShip()->getEps() < 1) {
				break;
			}
	...
	function beamFromColony() {
		...
		$this->addInformation(sprintf(_('Die %s hat folgende Waren von der Kolonie %s transferiert'),$this->getShip()->getName(),$target->getName()));
		foreach ($goods as $key => $value) {
			if ($this->getShip()->getEps() < 1) {
				break;
			}
	...
	protected function showRegionInfo() {
		...
		if ($this->getShip()->getCurrentMapField()->getMapRegion()->getDatabaseId() && !DatabaseUser::checkEntry($this->getShip()->getCurrentMapField()->getMapRegion()->getDatabaseId(),currentUser()->getId())) {
			DatabaseUser::addEntry($this->getShip()->getCurrentMapField()->getMapRegion()->getDatabaseId(),currentUser()->getId());
	...
	protected function dockShip() {
		...
		PM::sendPm(currentUser()->getId(),$target->getUserId(),'Die '.$this->getShip()->getName().' hat an der '.$target->getName().' angedockt',PM_SPECIAL_SHIP);
	...
	protected function payTradeLicence() {
		...
		switch ($mode) {
			case 'ship':
				...
				break;
			case 'account':
				$stor = TradeStorage::getStorageByGood($targetId,currentUser()->getId(),$tradepost->getLicenceCostGood()->getId());
				if ($stor == 0) {
					return;
				}
				...
				$stor->getTradePost()->lowerStorage(currentUser()->getId(),$tradepost->getLicenceCostGood()->getId(),$tradepost->calculateLicenceCost());
				break;
			default:
				return;
		}
	...
	protected function transferToAccount() {
		...
		foreach ($goods as $key => $value) {
			...
			if ($good->getGood()->isIllegal($tradepost->getTradeNetwork())) {
				$this->addInformation(sprintf(_('Der Handel mit %s ist in diesem Handelsnetzwerk verboten'),$good->getGood()->getName()));
				continue;
			}
			if ($count > $good->getCount()) {
				$count = $good->getCount();
	...
	protected function fleetLoadWarpcore($count) {
		...
		foreach($this->getShip()->getFleet()->getShips() as $key => $ship) {
			...
			if (!$load) {
				$this->addInformation(sprintf(_('%s: Zum Aufladen des Warpkerns werden mindestens 1 Deuterium sowie 1 Antimaterie benötigt'),$ship->getName()));
				continue;
			}
			$this->addInformation(sprintf(_('%s: Der Warpkern wurde um %d Einheiten aufgeladen'),$ship->getName(),$load));
		}
		DB()->commitTransaction();
		$this->addInformationMerge($msg);
	}
	...
	protected function salvageEpods() { #{{{
		...
		if ($dummy_crew->getCrew()->getUserId() != currentUser()->getId()) {
			PM::sendPm(currentUser()->getId(),$dummy_crew->getCrew()->getUserId(),sprintf(_('Der Siedler hat %d deiner Crewmitglieder von einem Trümmerfeld geborgen.'),$target->getCrew()),PM_SPECIAL_SHIP);
		}
	...
	function doCheck() {
		...
		if ($this->getShip()->getBuildplan()->getCrew() > 0 && $this->getShip()->getCrew() == 0) {
			if ($this->getShip()->getBuildplan()->getCrew() == 1) {
				...
				$this->errormsg = sprintf(_("Es werden %d Crewmitglieder benötigt"),$this->getShip()->getBuildplan()->getCrew());
			}	
			return;
		}
		$this->updateFields();
...
class NavPanel {
	...
	function getShipPosition() {
		if ($this->getShip()->getSystemsId() > 0) {
			return array("cx" => $this->getShip()->getSX(),
				     "cy" => $this->getShip()->getSY());
		}
		return array("cx" => $this->getShip()->getCX(),
			     "cy" => $this->getShip()->getCY());
	}

	function getMapBorders() {
		if ($this->getShip()->getSystemsId() > 0) {
			return array("mx" => $this->getShip()->getSystem()->getMaxX(),
				     "my" => $this->getShip()->getSystem()->getMaxY());
		}
		return array("mx" => MAP_MAX_X,
			     "my" => MAP_MAX_Y);
	}
