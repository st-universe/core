...
final class ActivatorDeactivatorHelper implements ActivatorDeactivatorHelperInterface
{
    ...
    private function activateIntern(
        ...
        try {
            ...
            $game->addInformation(sprintf(
                _('%s: [b][color=FF2626]System %s kann aufgrund Energiemangels (%dE benötigt) nicht aktiviert werden[/color][/b]'),
                $ship->getName(),
                $systemName,
                $e->getNeededEnergy()
            ));
    ...
    private function setAlertStateShip(ShipInterface $ship, int $alertState, GameControllerInterface $game): bool
    {
        ...
        try {
            ...
            $game->addInformation(sprintf(_('%s: [b][color=FF2626]Nicht genügend Energie um die Alarmstufe zu wechseln (%dE benötigt)[/color][/b]'), $ship->getName(), $e->getNeededEnergy()));
            return false;
        }
