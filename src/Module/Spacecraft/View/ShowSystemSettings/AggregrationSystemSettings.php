...
class AggregrationSystemSettings implements SystemSettingsProviderInterface
{
    ...
    public function setTemplateVariables(
        ...
        $mode1Commodities = array_filter($commodities, fn (array $entry): bool => $entry[4] === 1);
        $mode2Commodities = array_filter($commodities, fn (array $entry): bool => $entry[4] === 2);

        $mode1Commodities = array_map(fn (array $entry): array => [
        ...
        $mode2Commodities = array_map(fn (array $entry): array => [
            $this->commodityRepository->find($entry[0]),
            $this->commodityRepository->find($entry[1]),
            $entry[2],
            $entry[3]
        ...
        if ($module !== null && $module->getFactionId() == FactionEnum::FACTION_FERENGI) {
