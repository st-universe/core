...
function moveToPosition(posx, posy) {
	...
	actionToInnerContent('B_MOVE', `id=${spacecraftid}&posx=${posx}&posy=${posy}&sstr=${sstr}`);
}

function moveInDirection(action) {
	amount = document.shipform.navapp.value;
	actionToInnerContent(action, `id=${spacecraftid}&navapp=${amount}&sstr=${sstr}`);
}
...
function showETransferWindow(target) {
	closeAjaxWindow();
	openWindow('elt', 1);
	ajax_update('elt', '?id=' + spacecraftid + '&SHOW_ETRANSFER=1&target=' + target);
}

function showSelfdestructWindow(target) {
	closeAjaxWindow();
	openWindow('elt', 1, 300);
	ajax_update('elt', '?id=' + spacecraftid + '&SHOW_SELFDESTRUCT_AJAX=1&target=' + target);
}
...
function showSectorScanWindow(obj, x, y, sysid, loadSystemSensorScan) {
	...
	if (x && y) {
		ajax_update('elt', 'station.php?id=' + spacecraftid + '&SHOW_SENSOR_SCAN=1&x=' + x + '&y=' + y + '&systemid=' + sysid);
		if (loadSystemSensorScan) {
			ajax_update('systemsensorscan', 'station.php?id=' + spacecraftid + '&SHOW_SYSTEM_SENSOR_SCAN=1&x=' + x + '&y=' + y);
		}
	} else {
...
function openTradeMenu(postid) {
	closeAjaxWindow();
	openPJsWin('elt', 1);
	ajax_update('elt', '?SHOW_TRADEMENU=1&id=' + spacecraftid + '&postid=' + postid);
}
function switchTransferFromAccount(postid) {
	ajax_update('trademenutransfer', '?SHOW_TRADEMENU_TRANSFER=1&id=' + spacecraftid + "&mode=from&postid=" + postid);
...
function switchTransferToAccount(postid) {
	ajax_update('trademenutransfer', '?SHOW_TRADEMENU_TRANSFER=1&id=' + spacecraftid + "&mode=to&postid=" + postid);
...
function showRegionInfo(region) {
	closeAjaxWindow();
	openPJsWin('elt', 1);
	ajax_update('elt', '?SHOW_REGION_INFO=1&id=' + spacecraftid + '&regionid=' + region);
}
...
function renameCrew(crew_id) {
	...
	ajax_update('rn_crew_' + crew_id, '?B_RENAME_CREW=1&id=' + shipid + '&crewid=' + crew_id + '&' + Form.Element.serialize('rn_crew_' + crew_id + '_value'));
}
