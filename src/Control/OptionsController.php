...
final class OptionsController extends GameController
{
    ...
    {
        DB()->query("UPDATE stu_user_profiles SET " . $field . "='" . addslashes($value) . "' WHERE user_id=" . $this->getUser()->getId(),
            6);
    }
    ...
    {
        if ($this->db->query("SELECT delmark FROM stu_user WHERE id=" . $user, 1) != 1) {
            die(show_error(902));
        }
    }
    ...
    {
        $result = $this->db->query("SELECT id FROM stu_user WHERE id=" . $arr['user'] . " AND pass='" . md5($arr['pass']) . "' LIMIT 1",
            1);
        if ($result == 0) {
            exit();
        }
    ...
    function sendDeletionMail()
    {
        ...
        mail($this->getUser()->email, "Star Trek Universe Accountlöschung",
            "Hallo " . strip_tags($this->getUsername()) . "\n\n
...
Content-Type: text/plain; charset=UTF-8;");
    ...
    {
        $settings = array(
            "email_not" => "setEmailNotification",
