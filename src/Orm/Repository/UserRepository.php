            ...
            sprintf(
                'SELECT u FROM %s u
                 ...
                 AND (u.delmark = :deletionMark
                        OR (not u.vac_active AND u.lastaction > 0 AND u.lastaction < :idleTimeThreshold)
                        OR (u.vac_active AND u.lastaction > 0 AND u.lastaction < :idleTimeVacationThreshold)
                    )
                 ORDER BY u.id ASC',
                User::class
            )
